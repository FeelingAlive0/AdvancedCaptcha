...
public class CaptchaManager {
    ...
    public void startCaptcha(Player player) {
        ...
        if (verifiedPlayers.containsKey(playerId)) {
            if (currentTime - verifiedPlayers.get(playerId) < configManager.getCooldown()) {
    ...
    public void handleClick(Player player, String clickValue) {
        ...
        if (isCorrect) {
            ...
            log(Level.WARNING, messagesManager.getMessage("log-failed-attempt", player.getName(), session.getIp(), "Wrong button"));
            // Repeat the same captcha instead of creating a new session
    ...
    private void startSessionCleanup() {
        new BukkitRunnable() {
            ...
            public void run() {
                long currentTime = System.currentTimeMillis();
                synchronized (sessions) {
                    sessions.entrySet().removeIf(entry -> currentTime - entry.getValue().getCreationTime() > configManager.getTimeout());
                }
            }
    ...
    private boolean isValidIp(String sessionIp, String currentIp) {
        // Placeholder for VPN/proxy detection (e.g., via BungeeCord or external API like ip-api.com)
        // For simplicity, compare IPs directly; extend with API call if needed
