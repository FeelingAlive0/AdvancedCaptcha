...
public class ButtonCaptcha implements CaptchaTypeHandler {
    ...
    public void showCaptcha(Player player, CaptchaSession session) {
        ...
        String buttonText = messagesManager.getButtonText((int) (Math.random() * messagesManager.getButtonTexts().size()));
        ...
        for (int i = 0; i < buttonCount; i++) {
            ...
            if (i == correctButton) {
                ...
                button = new TextComponent(messagesManager.getButtonText((int) (Math.random() * messagesManager.getButtonTexts().size())));
                color = buttonColor;
            } else {
                button = new TextComponent(messagesManager.getButtonText((int) (Math.random() * messagesManager.getButtonTexts().size())));
